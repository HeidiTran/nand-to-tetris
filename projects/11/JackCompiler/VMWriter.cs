...
namespace JackCompiler
{
	...
	class VMWriter
	{
		...
		/// </summary>
		public void WriteLabel(string label) => WriteL($"label {label}");
		...
		/// </summary>
		public void WriteGoto(string label) => WriteL($"goto {label}");

		/// <summary>
		/// This command effects a conditional goto operation.
		/// The stack's topmost value is popped if the val is NOT zero, 
		...
		/// </summary>
		public void WriteIf(string label) => WriteL($"if-goto {label}");
		...
		public void WriteCall(string name, int nArgs) => WriteL($"call {name} {nArgs}");
		...
		/// </summary>
		public void WriteReturn() => WriteL("return");
		...
		private void WriteL(string text) => _streamWriter.WriteLine(text);
	}
}
